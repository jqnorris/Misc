...
int main(int argc, char **argv)
{
	...
	std::set<unsigned int> left_side;
	for(j=-L/2; j < L/2; j++)
	{

		current_cell = &lattice[std::make_pair(j,-L/2+1)];
		if(current_cell->clusterN != 0)
		{
			left_side.insert(find(current_cell->clusterN));
		}
	...
	for(j=-L/2; j < L/2; j++)
	{
		...
		current_cell = &lattice[std::make_pair(j,L/2)];
		if(left_side.count(find(current_cell->clusterN)) > 0)
		{
			spanning = 1;
			break;
		}
