	...
	for(short int dir=0; dir < 4; dir++)
	{
		...
		if(new_site < lattice_size)
		{
			if(lattice[new_site])
			{
				...
				if(get_bond_strength(this_bond) < 0.5)
				{
...
void clean_up_rupture()
{
	...
	for(iterator= ruptured_bonds.begin(); iterator != ruptured_bonds.end(); iterator++)
	{
		...
		bond_strength_table[get_bond_index(iterator->second)] = 1;
	}
	ruptured_bonds.clear();
}

unsigned long int number_of_bonds = 2* lattice_size - lattice_height - lattice_length;

bool quiet()
{
	unsigned long int this_one = number_of_bonds*boost_drand48();

	bond_strength_table[this_one] = bond_strength_table[this_one]-0.01;
...
int main(int argc, char **argv)
{
	...
	unsigned long int number_of_runs = atoi(argv[3]);
	...
	for(i=0; i < number_of_runs; i++)
	{
		...
		this_rupture_size = ruptured_bonds.size();
	...
	toFile1 << "Rupture sizes for: " << number_of_runs << " runs of " << lattice_height << " by " << lattice_length << "\n";
	...
	toFile2 << number_of_runs << " events for: " << lattice_height << " by " << lattice_length << "\n";
