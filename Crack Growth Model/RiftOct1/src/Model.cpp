...
void Model::timeStep()
{
	...
	if (array[currentElement.x][currentElement.y].gap + deltaGap <= 0)
	{
		deltaGap = -array[currentElement.x][currentElement.y].gap;
	}
...
double Model::freeEnergy(double deltaGap)
{
	...
	double quadraticTerm = currentOptions.quarticStrength * pow(Gap, 4);
