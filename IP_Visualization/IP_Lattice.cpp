...
short int L = 0;
short int pix_x(int x)
{
    return x+L/2;
}

short int pix_y(int y)
{
    return -y+L/2;
}
...
int main(int argc, char **argv)
{
    ...
    QImage image(L, L, QImage::Format_RGB888);
    image.fill(qRgb(255, 255, 255));
    const QPoint offset = QPoint(-L/2, -L/2+1);
    image.setOffset(offset);

    QLabel label;
    label.setPixmap(QPixmap::fromImage(image).scaledToWidth(500));
    label.show();
    ...
    try
    {
        for (i=0; i<N;)
        {
            ...
            if(!already_invaded)
            {
                x = get_end(weakest->second).first;
                y = get_end(weakest->second).second;
                image.setPixel(pix_x(x), pix_y(y),qRgb( 25, 178, 255));
                label.setPixmap(QPixmap::fromImage(image).scaledToWidth(500));
                label.show();
 //               if(weakest->first > 0.5)
//                {
 //                   msDelay(2000);
//                }
//                else
