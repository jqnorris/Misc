...
{
    static int next_strength;

	if(direction == 0 or direction == 1)
	{
        return anisotropy*next_strength++;
	}
	else
	{
...
std::deque<std::tuple<int, int, long int> > burst_list;
std::deque<std::tuple<int, int, int, long int> > burst_array;
		...
		if(!already_invaded)
		{
			...
			else
			{
				if(counter != 0)
				{
                    ...
                    burst_list.push_back(std::make_tuple(hypocenter.first.first, hypocenter.first.second, counter));
					counter = 0;
				}
			}
...
void record_sim(int i)
{
    ...
    std::deque<std::tuple<int, int, long int> >::iterator burst_iter = burst_list.begin();
    std::deque<std::tuple<int, int, long int> >::iterator burst_list_end = burst_list.end();
	...
	{
        burst_x = std::get<0>(*burst_iter);
        burst_y = std::get<1>(*burst_iter);
        burst_size = std::get<2>(*burst_iter);

        burst_array.push_back(std::make_tuple(i, burst_x, burst_y, burst_size));
...
void write_runs_to_file(void)
{
	...
	std::ofstream toFile6("bursts.txt", std::ios::trunc);
        ...
        std::deque<std::tuple<int, int, int, long int> >::iterator burst_iter = burst_array.begin();
        std::deque<std::tuple<int, int, int, long int> >::iterator burst_list_end = burst_array.end();

		while(burst_iter != burst_list_end)
		{
            toFile6 << std::get<0>(*burst_iter) << "\t";
            toFile6 << std::get<1>(*burst_iter) << "\t";
            toFile6 << std::get<2>(*burst_iter) << "\t";
            toFile6 << std::get<3>(*burst_iter) << "\n";
