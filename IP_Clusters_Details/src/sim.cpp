...
std::deque<std::pair<double, short int> > type_of_growth;
...
bool grow_clusters(Bond bond, double strength)
{
	...
	if(two == -1)
	{
		if(one == 0)
		{
			...
			type_of_growth.push_back(std::make_pair(strength, -1));
			new_cluster_number++;
		}
		return true;
	}
	if( (one != two || one == 0))
	{
		...
		case 1:
			...
			type_of_growth.push_back(std::make_pair(strength, 0));
			return true;
		case 2:
...
int main(int argc, char **argv)
{
	...
	{
		lattice[weakest->second.first].is_open[weakest->second.second] = grow_clusters(weakest->second, weakest->first);
	...
	lattice_iterator cluster_iter;
	for(cluster_iter = lattice.begin(); cluster_iter != lattice.end(); cluster_iter++)
	{
		begin = cluster_iter->first;
		Line = cluster_iter->second;
		if(Line.is_open[0])
		{
	...
	std::deque<Site> largest_cluster_sites;
	Site this_site;
	for(cluster_iter = lattice.begin(); cluster_iter != lattice.end(); cluster_iter++)
	{
		if(cluster_iter->second.clusterN == largest_cluster)
		{
			this_site = cluster_iter->first;
			largest_cluster_sites.push_back(this_site);
		}
	}

	std::ofstream toFile3("largest_cluster.txt", std::ios::trunc);
	toFile3 << largest_cluster_sites.size() << "\n";

	while (!largest_cluster_sites.empty())
	{
		this_site = largest_cluster_sites.front();
		largest_cluster_sites.pop_front();
		toFile3 << this_site.first << "\t" << this_site.second << "\n";
	}
	...
	std::ofstream toFile5("type_of_growth.txt", std::ios::trunc);
	toFile5 << type_of_growth.size() << "\n";
	while (!type_of_growth.empty())
	{
		toFile5 << type_of_growth.front().first << "\t" << type_of_growth.front().second << "\n";
		type_of_growth.pop_front();
	}
